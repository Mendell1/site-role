/* ============================================================
   ...
   ------------------------------------------------------------
   A versão qrcode@1.5.4 não disponibiliza mais o bundle /build
   no CDN usado pelo projeto. Este adaptador mantém a API
   QRCode.toCanvas esperada pelo módulo de check-in e usa
   qrcodejs@1.0.0 como gerador visual, com fallback de CDN.
   ============================================================ */
(() => {
  'use strict';
  ...
  const apiCompat = {
    async toCanvas(canvas, texto, opcoes = {}) {
      if (!(canvas instanceof HTMLCanvasElement)) {
        throw new Error('Destino do QR precisa ser um canvas');
      }
  ...
  function carregarBiblioteca() {
    ...
    carregamento = new Promise((resolve, reject) => {
      carregarFonte(0, resolve, reject);
    }).catch(erro => {
      carregamento = null;
      window.QRCode = apiCompat;
      throw erro;
